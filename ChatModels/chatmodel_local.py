...
def markdown_chat(prompt: str):
    ...
    cache_key = create_cache_key(prompt=prompt, model_name=model_name, temperature=0)
    ...
    if cached_response is not None:
        console.print(
            Panel(
                Markdown(cached_response), title="cache hit", border_style="bold green"
            )
        )
        return
    ...
    console.print(
        Panel(
            ...
            title=f"Generated Response via {model_name}",
        )
    )
...
def cache_stats():
    ...
    console.print(
        Panel(
            f"cached responses: {cache.count()}",
            border_style="green",
            title="Cache Count",
        )
    )
